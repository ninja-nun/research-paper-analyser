...
const BASE = '/api';  // proxied by Vite to localhost:8000

export const api = {

  // Upload a PDF — returns the paper_id the backend assigned
  ...
  // The backend only tracks one paper at a time — no list endpoint
  // Return a single-item list from the /paper endpoint
  ...
  deletePaper: async (_id: string): Promise<void> => {
    // backend has no delete endpoint yet — no-op for now
  },
};

// Map backend JSON shape to the frontend Paper type
